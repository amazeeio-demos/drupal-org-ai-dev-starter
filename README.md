...
A [Drupal CMS](https://www.drupal.org/project/cms) 2.x demo on [Lagoon](https://lagoon.sh) — a starting point showing the full [amazee.ai](https://amazee.ai) + [Drupal AI](https://www.drupal.org/project/ai) toolkit: AI-enhanced CKEditor widgets (summarize, style-guide, alt-text generation), AI search, and webforms with their JavaScript libraries.

## How it's built

The whole site is defined by the bundled recipe at `recipes/drupal-org-ai-dev-starter/` — Drupal CMS starter plus the [amazee.ai CKEditor Widgets](https://www.drupal.org/project/amazeeio_ckeditor_widgets) recipe (the `content_with_ai` text format and AI automator chains), the `ai_search` and `webform` modules, and demo content as YAML. `drush site:install <recipe>` produces the complete demo from a clean database. Webform's front-end libraries (CodeMirror, Select2, Tippy.js, etc.) are declared as Composer `package` repositories so they land in `web/libraries/`.
